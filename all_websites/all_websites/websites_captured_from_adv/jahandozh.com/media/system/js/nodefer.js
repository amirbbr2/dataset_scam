...
DOM.attributes(el,{'href':img,'title':title.replace(/_/,' '),'onclick':''});DOM.addClass(el,'spopup');}}});},convertLightbox:function(){var each=JCEMediaBox.each,DOM=JCEMediaBox.DOM;each(DOM.select('a[rel*=lightbox]'),function(el){DOM.addClass(el,'spopup');r=el.rel.replace(/lightbox\[?([^\]]*)\]?/,function(a,b){if(b){return'group['+b+']';}
return'';});DOM.attribute(el,'rel',r);});},convertShadowbox:function(){var each=JCEMediaBox.each,DOM=JCEMediaBox.DOM;each(DOM.select('a[rel*=shadowbox]'),function(el){DOM.addClass(el,'spopup');r=el.rel.replace(/shadowbox\[?([^\]]*)\]?/,function(a,b){var attribs='',group='';if(b){group='group['+b+']';}
...
t.setCookie('jcemediabox_'+key+'_'+i,1,dts);t.start(el);}}else if(el.auto=='multiple'){t.start(el);}}});},init:function(){window.spopup=this;this.create();},getPopups:function(s,p){var selector='a.jcebox, a.jcelightbox, a.spopup, area.jcebox, area.jcelightbox, area.spopup';return JCEMediaBox.DOM.select(s||selector,p);},getData:function(n){var DOM=JCEMediaBox.DOM,o={},data;var re=/\w+\[[^\]]+\]/;data=DOM.attribute(n,'data-mediabox')||DOM.attribute(n,'data-json');if(!data){var title=DOM.attribute(n,'title');var rel=DOM.attribute(n,'rel');if(title&&re.test(title)){o=this.params(title);DOM.attribute(n,'title',o.title||'');return o;}
...
if(/(jcelightbox|jcebox)/.test(el.className)){DOM.removeClass(el,'jcelightbox');DOM.removeClass(el,'jcebox');DOM.addClass(el,'spopup');}
...
/*Media Box*/
/*Animate JS*/
var onScrollOffset = {scrollOffset: 75};
onScrollOffset.get = function(){return scrollOffset;};
onScrollOffset.set = function(offset){scrollOffset = offset;};
(function($) {jQuery(document).ready(function(){var animationStyleClasses = ["animated", "infinite", "bounce", "flash", "pulse", "rubberBand", "shake", "swing", "tada", "wobble", "bounceIn", "bounceInDown", "bounceInLeft", "bounceInRight", "bounceInUp", "bounceOut", "bounceOutDown", "bounceOutLeft", "bounceOutRight", "bounceOutUp", "fadeIn", "fadeInDown", "fadeInDownBig", "fadeInLeft", "fadeInLeftBig", "fadeInRight", "fadeInRightBig", "fadeInUp", "fadeInUpBig", "fadeOut", "fadeOutDown", "fadeOutDownBig", "fadeOutLeft", "fadeOutLeftBig", "fadeOutRight", "fadeOutRightBig", "fadeOutUp", "fadeOutUpBig", "flip", "flipInX", "flipInY", "flipOutX", "flipOutY", "lightSpeedIn", "lightSpeedOut", "rotateIn", "rotateInDownLeft", "rotateInDownRight", "rotateInUpLeft", "rotateInUpRight", "rotateOut", "rotateOutDownLeft", "rotateOutDownRight", "rotateOutUpLeft", "rotateOutUpRight", "hinge", "rollIn", "rollOut", "zoomIn", "zoomInDown", "zoomInLeft", "zoomInRight", "zoomInUp", "zoomOut", "zoomOutDown", "zoomOutLeft", "zoomOutRight", "zoomOutUp"];var delayClasses  = ["delay1", "delay2", "delay3", "delay4", "delay5", "delay6", "delay7", "delay8", "delay9", "delay10", "delay11", "delay12"];var durationClasses  = ["duration1", "duration2", "duration3", "duration4", "duration5", "duration6", "duration7", "duration8", "duration9", "duration10", "duration11", "duration12", "duration13", "duration14", "duration15", "duration16", "duration17", "duration18", "duration19", "duration20"];var scrollOffset = onScrollOffset.get();
jQuery('.eds-on-scroll').each(function(){var $module = jQuery(this);
if($module.hasClass('custom')){$module.removeClass(function(index, className){var removeClassList = "";var classList = className.trim().split(/\s+/);jQuery.each(classList, function(index, value){if((value=='eds-on-scroll') 
||(jQuery.inArray(value, animationStyleClasses)!==-1) 
|| (jQuery.inArray(value, delayClasses)!==-1))
removeClassList += " " + value;});
return removeClassList;});}else{var classesToAdd = " eds-scroll-visible ";$module.removeClass(function(index, className){var removeClassList = "";var classList = className.trim().split(/\s+/);jQuery.each(classList, function(index, value){if((jQuery.inArray(value, animationStyleClasses)!==-1) 
|| (jQuery.inArray(value, delayClasses)!==-1))
removeClassList += " " + value;});classesToAdd += removeClassList;return removeClassList;});$module.addClass("eds-scroll-hidden");$module.viewportChecker({classToAdd: classesToAdd,
offset: scrollOffset });}});jQuery('.eds-on-click').each(function(){var $module = jQuery(this);if($module.hasClass('custom')){$module.removeClass(function(index, className){var removeClassList = "";var classList = className.trim().split(/\s+/);jQuery.each(classList, function(index, value){if((value=='eds-on-click') 
||(jQuery.inArray(value, animationStyleClasses)!==-1) 
|| (jQuery.inArray(value, delayClasses)!==-1))
removeClassList += " " + value;});return removeClassList;});}else{var classesToAdd = " ";$module.removeClass(function(index, className){var removeClassList = "";var classList = className.trim().split(/\s+/);jQuery.each(classList, function(index, value){if((jQuery.inArray(value, animationStyleClasses)!==-1) 
|| (jQuery.inArray(value, delayClasses)!==-1)
|| (jQuery.inArray(value, durationClasses)!==-1))
removeClassList += " " + value;});classesToAdd += removeClassList;return removeClassList;});$module.click(function (){jQuery(this).removeClass(classesToAdd).addClass(classesToAdd).one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){jQuery(this).removeClass(classesToAdd);});});}});jQuery('.eds-on-hover').each(function(){var $module = jQuery(this);if($module.hasClass('custom')){$module.removeClass(function(index, className){var removeClassList = "";var classList = className.trim().split(/\s+/);jQuery.each(classList, function(index, value){if((value=='eds-on-hover') 
||(jQuery.inArray(value, animationStyleClasses)!==-1) 
|| (jQuery.inArray(value, delayClasses)!==-1))
removeClassList += " " + value;});return removeClassList;});}else{var classesToAdd = " ";$module.removeClass(function(index, className){var removeClassList = "";var classList = className.trim().split(/\s+/);jQuery.each(classList, function(index, value){if((jQuery.inArray(value, animationStyleClasses)!==-1) 
|| (jQuery.inArray(value, delayClasses)!==-1)
|| (jQuery.inArray(value, durationClasses)!==-1))
removeClassList += " " + value;});classesToAdd += removeClassList;return removeClassList;});var hovered = false;$module.hover(
function (){hovered = true;jQuery(this).removeClass(classesToAdd).addClass(classesToAdd).one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){jQuery(this).removeClass(classesToAdd);});},
function (){hovered = false;jQuery(this).on('webkitAnimationIteration oanimationiteration msAnimationiteration animationiteration', function(e){if(!hovered){jQuery(this).removeClass(classesToAdd);}});});}});});})(jQuery);
/*Animate JS*/
