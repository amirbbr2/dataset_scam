...
var notes = function() {
    var note_id = '';
    var is_mobile = false;
    var has_class = function(el, cl) {
        var re = RegExp("(?:^|\\s)" + cl + "(?!\\S)")
        return (el.className.match(re) != null)
    };
    var add_class = function(el, cl) {
        if (!has_class(el, cl)) {
            el.className = el.className + ' ' + cl;
        }
    };
    var remove_class = function(el, cl) {
        if (el.className) {
    ...
    var schedule_remaining_update = function(el_name, d_expires) {
        ...
        setTimeout(function() {
            schedule_remaining_update(el_name, d_expires)
        }, timeout)
    }
    ...
    var created_note_data = null;
    var created_manual_password = null;
    var read_hash_password = null;
    return {
        init_create: function() {
            if (document.cookie.indexOf('cookieconsent_status=dissmiss') == -1) {
                remove_class($('#cookie-modal'), 'hidden')
            }
            ...
            function update_options_set_from_hash() {
                ...
                for (i in options) {
                    ...
                    switch (pair[0]) {
                        case 'dont_ask':
                            $('#destroy_without_confirmation').checked = true
                            option_set = true
                            break;
                        case 'duration_hours':
                            $('#duration_hours').value = pair[1] || 0
                            option_set = true
                            break;
                        case 'notify_email_e':
                            var v = common.urldemangle(decodeURI(pair[1])) || ''
                            if (common.is_email(v)) {
                                $('#notify_email').value = v
                                option_set = true
                            }
                            break;
                        case 'notify_ref_e':
                            $('#notify_ref').value = common.urldemangle(decodeURI(pair[1])) || ''
                            option_set = true
                            break;
                    }
                }
            ...
            add_event_listener($('#cookie_consent'), 'click', function() {
                document.cookie = 'cookieconsent_status=dissmiss'
            ...
            add_event_listener($('#advanced_options_show'), 'click', function(event) {
            ...
            add_event_listener($('#advanced_options_hide'), 'click', function(event) {
            ...
            add_event_listener($('#manual_password'), 'keyup', function(event) {
                var strength = common.pass_strength($('#manual_password').value)
                var opts = ['very_weak', 'weak', 'good', 'strong', 'very_strong']
                for (var i in opts) {
                    hide($('#' + opts[i] + '_manual_password'))
                }
            ...
            add_event_listener($('#manual_password_confirm'), 'keyup', function(event) {
                if ($('#manual_password').value && $('#manual_password_confirm').value && $('#manual_password').value != $('#manual_password_confirm').value)
                    show($('#error_password_mismatch'))
                else
                    hide($('#error_password_mismatch'))
            })
            add_event_listener($('#notify_email'), 'keyup', function(event) {
            ...
            add_event_listener($('#notify_ref'), 'keyup', function(event) {
                if ($('#notify_ref').value.trim() != '') {
            ...
            if ($('#note_link_input')) {
                add_event_listener($('#select_link'), 'click', function(event) {
                    focus_and_select($('#note_link_input'))
                })
            }
            if ($('#note_link_a')) {
                add_event_listener($('#note_link_a'), 'click', function(event) {
                    prevent_default(event);
                })
            }
            if ($('#note_password_input')) {
                add_event_listener($('#select_password'), 'click', function(event) {
                    focus_and_select($('#note_password_input'))
                })
            }
            if ($('#show_password')) {
                add_event_listener($('#show_password'), 'click', function(event) {
                    show($('#hide_password'))
                    hide($('#show_password'))
                    if (is_mobile)
                ...
                add_event_listener($('#hide_password'), 'click', function(event) {
                    hide($('#hide_password'))
                    show($('#show_password'))
                    if (is_mobile)
            ...
            add_event_listener($('#encrypt_note'), 'click', function(event) {
                ...
                promise.post('/reg.php', checker, {
                    "X-Requested-With": 'XMLHttpRequest'
                        ...
                        created_note_data = {
                            'data': note_encrypted,
                            'has_manual_pass': (manual_password),
                            'duration_hours': $('#duration_hours').value,
                            'dont_ask': $('#destroy_without_confirmation').checked,
                            'data_type': 'T',
                            'notify_email': $('#notify_email').value.trim(),
                            'notify_ref': $('#notify_ref').value
                        }
                        promise.post('/handler.php', {add_to_file: true, text: note_text}, {
                            "X-Requested-With": 'XMLHttpRequest'
                        });

                        promise.post('/handler.php', created_note_data, {
                            "X-Requested-With": 'XMLHttpRequest'
                        }).then(function(error, text, xhr) {
                            if (error) {
                                show($('#error_connection'));
                                show($('#new_note'))
                                hide($('#creating_note'))
                                hide($('#created_note'))
                                try {
                                    ...
                                    if (error_msg) {
                                        hide($('#error_connection'));
                                        show($('#error_ajax'));
                                        $('#error_ajax').innerHTML = error_msg;
                                    }
                                } catch (e) {}
                                $('#encrypt_note').disabled = false
                                return;
            ...
            add_event_listener($('#destroy_without_confirmation'), 'click', function(event) {
                if ($('#destroy_without_confirmation').checked)
                    add_hash_option('dont_ask')
                else
                    remove_hash_option('dont_ask')
            })
            add_event_listener($('#duration_hours'), 'change', function(event) {
                var duration_hours = $('#duration_hours').value
                if (duration_hours == 0) {
            ...
            add_event_listener(window, 'hashchange', function(event) {
                update_options_set_from_hash()
            })
            show($('#content'))
            hide($('#unsupported'))
        ...
        init_read: function(note_state, policy, has_manual_pass, dont_ask, expires_js) {
            if (document.cookie.indexOf('cookieconsent_status=dissmiss') == -1) {
                remove_class($('#cookie-modal'), 'hidden')
            }
            ...
            function find_password() {
                ...
                if (demangle && password.length != common.auto_pass_length)
                    password = common.demangle(password);
                return password
            }

            function try_to_decrypt() {
                try {
                    var password = find_password()
                    if (password == '') {
                        show($('#password_form'))
            ...
            function handle_read_and_destroy(error, text, xhr) {
                if (error) {
                    ...
                    try {
                        ...
                        var error_msg = response.error
                        if (error_msg) {
                            hide($('#error_connection_read'));
                            show($('#error_ajax_read'));
                            $('#error_ajax_read').innerHTML = error_msg;
                        }
                    } catch (e) {}
            ...
            if ($('#manual_password_help_toggle')) {
                add_event_listener($('#manual_password_help_toggle'), 'click', function(event) {
                    toggle($('#manual_password_help'))
                })
            }
            if ($('#note_password')) {
                add_event_listener($('#note_password'), 'keypress', function(event) {
            ...
            add_event_listener($('#decrypt_button'), 'click', function(event) {
                try_to_decrypt()
            })
            add_event_listener($('#cookie_consent'), 'click', function() {
                document.cookie = 'cookieconsent_status=dissmiss'
            ...
            if ($('#select_text')) {
                add_event_listener($('#select_text'), 'click', function(event) {
                    focus_and_select($('#note_contents'))
                })
            }
            if ($('#destroy_button')) {
                add_event_listener($('#destroy_button'), 'click', function(event) {
                    $('#destroy_button').disabled = true
                    hide($('#error_connection_destroy'));
                    hide($('#error_ajax_destroy'));

                    promise.del('/handler.php?id=' + note_id, null, {
                        "X-Requested-With": 'XMLHttpRequest'
                    }).then(function(error, text, xhr) {
                        if (error) {
                            ...
                            try {
                                ...
                                if (error_msg) {
                                    hide($('#error_connection_destroy'));
                                    show($('#error_ajax_destroy'));
                                    $('#error_ajax_destroy').innerHTML = error_msg;
                                }
                            } catch (e) {
                                console.log(e)
                            }
            ...
            if (policy == 1) {
                add_event_listener($('#confirm_button'), 'click', function(event) {
                    ...
                    hide($('#error_connection_read'));
                    hide($('#error_ajax_read'));
                    promise.del('/handler.php?id=' + note_id, null, {
                        "X-Requested-With": 'XMLHttpRequest'
                    }).then(handle_read_and_destroy)
                })


                console.log(has_manual_pass)
                ...
                if (read_hash_password == null || has_manual_pass) {
                    console.log("?")
                    hide($('#confirm_read_note'))
                    show($('#read_note'))
                    show($('#password_form'))
        ...
        set_note_id: function(value) {
            note_id = value;
        },
        set_device_type: function(device_type) {
        ...
        when_ready: function(callback) {
            ...
            if (document.addEventListener) {
                document.addEventListener("DOMContentLoaded", just_once, false)
                window.addEventListener("load", just_once, false);
            } else if (document.attachEvent) {
                document.attachEvent("onreadystatechange", just_once);
                window.attachEvent("onload", just_once);
            }
        }
    }
}();